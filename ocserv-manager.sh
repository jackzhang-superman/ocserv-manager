...
configure_ifb() {
    ...
    tc class replace dev "$IFB_DEVICE" parent 1: classid 1:10 htb rate "$RATE" ceil "$RATE"
}
...
limit_interface() {
    ...
    tc filter replace dev "$iface" parent ffff: protocol all pref 10 u32 \
...
limit_daemon() {
    ...
    while true; do
        ...
        while IFS= read -r iface; do
            [[ -n "$iface" ]] || continue
            if ! tc class show dev "$iface" 2>/dev/null | grep -q "rate $RATE"; then
